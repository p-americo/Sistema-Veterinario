...
export class AgendamentoNovoComponent implements OnInit {
  ...
  @Input() servicos: ServicoResponse[] = [];
  @Input() isLoading = false;
  @Input() mensagemSucesso: string | null = null;
  @Input() mensagemErro: string | null = null;

  @Output() confirmar = new EventEmitter<AgendamentoRequest>();
  ...
  onPetSelecionado(event: Event): void {
    ...
    if (this.petSelecionado && this.petSelecionado.id) {
      const url = `${environment.apiUrl}/api/animais/imagem/${this.petSelecionado.id}`;
  ...
  confirmarAgendamento(): void {
    ...
    this.confirmar.emit(agendamentoRequest);
  }
  ...
  isDiaPassado(data: Date): boolean {
    return data < this.hoje;
  }

  isFormularioValido(): boolean {
    return !!this.petSelecionado && !!this.servicoSelecionadoId && !!this.diaSelecionado && !!this.horarioSelecionado;
  }

  gerarDiasDoMes(): void {
    this.diasDoMes = [];
    const primeiroDiaDoMes = new Date(this.mesAtual.getFullYear(), this.mesAtual.getMonth(), 1);
    const ultimoDiaDoMes = new Date(this.mesAtual.getFullYear(), this.mesAtual.getMonth() + 1, 0);
    
    for (let i = primeiroDiaDoMes.getDay(); i > 0; i--) {
      const data = new Date(primeiroDiaDoMes);
      data.setDate(data.getDate() - i);
      this.diasDoMes.push({ data, outroMes: true });
    }
    
    for (let i = 1; i <= ultimoDiaDoMes.getDate(); i++) {
      const data = new Date(this.mesAtual.getFullYear(), this.mesAtual.getMonth(), i);
      this.diasDoMes.push({ data, outroMes: false });
    }
    
    const ultimoDiaGrid = this.diasDoMes[this.diasDoMes.length - 1].data;
    const diasParaCompletar = 6 - ultimoDiaGrid.getDay();
    for (let i = 1; i <= diasParaCompletar; i++) {
      const data = new Date(ultimoDiaGrid);
      data.setDate(data.getDate() + i);
      this.diasDoMes.push({ data, outroMes: true });
    }
  }
  ...
  isDiaSelecionado(data: Date): boolean {
